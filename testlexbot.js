...
function checkSlotValues(botName, botAlias, sequence, interaction, lexParams, lexResponse, requestId) {
	...
	if (
		postConditions.slots &&
		postConditions.slots.length > 0 &&
		postConditions.intentName &&
		postConditions.intentName !== ''
	) {
		...
		if (lexResponse.intentName == postConditions.intentName) {
			...
			postConditions.slots.forEach(function (slot) {
				if (
					!(slot.slotValue == null) &&
					lexResponse.slots[slot.slotName] == null &&
					(lexResponse.slots[slot.slotName] == null ||
						!lexResponse.slots[slot.slotName].match(slot.slotValue))
				) {
					// record a failure
					recordFailure(
						botName,
						...
						lexResponse,
						requestId,
						`Slot value ${slot.slotName} did not match (actual) [${
							lexResponse.slots[slot.slotName]
						}] !~= (expected) [${slot.slotValue}] for [${interaction.utterance}]`
					);
					succeeded = false;
				} else {
					console.log(
						`I  [${sequence.name}/${interaction.utterance}] Acceptable slot value found - ${
							slot.slotName
						} / ${lexResponse.slots[slot.slotName]}`
					);
				}
		...
		recordFailure(
			...
			lexResponse,
			requestId,
			`Either slot values or Intent name were not provided, so did not match [${JSON.stringify(
				postConditions.slots
			)}]`
		);
		return false;
		...
		if (lexResponse.dialogState == postConditions.dialogState) {
			...
			if (postConditions.intentName && postConditions.intentName !== '') {
				if (postConditions.intentName != lexResponse.intentName) {
					...
					console.log(
						`I  [${sequence.name}/${interaction.utterance}] Acceptable dialogState found - ${lexResponse.dialogState} / ${lexResponse.intentName}`
					);
					return true;
				}
			} else return true;
...
console.log(
	`I Running test cases from ${testConfigFile} for ${testConfig.botName}:${testConfig.botAlias} - ${
		testConfig.name ? testConfig.name : ''
	}`
);
